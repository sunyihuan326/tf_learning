...
BASE_DIR = "/Users/sunyihuan/PycharmProjects/myself/tf_learning/learning_begin"
...
if __name__ == '__main__':
    # 训练MNIST的estimator
    mnist_model_dir = "/Users/sunyihuan/PycharmProjects/myself/tf_learning/learning_begin/mnist_model"
    ...
    train_labels = np.argmax(train_labels, axis=1)  # one_hot格式转为（？，1）
    eval_data = mnist.test.images
    eval_labels = np.asarray(mnist.test.labels, dtype=np.int32)
    eval_labels = np.argmax(eval_labels, axis=1)  # one_hot格式转为（？，1）

    # 训练
    train_input_fn = tf.estimator.inputs.numpy_input_fn(
        x={"x": train_data},
        y=np.asarray(train_labels),
        batch_size=8, num_epochs=2, shuffle=True
    )
    ...
    eval_input_fn = tf.estimator.inputs.numpy_input_fn(
        ...
        batch_size=8, num_epochs=1, shuffle=True
    )
    ...
    mnist_transfer_model_dir = "/Users/sunyihuan/PycharmProjects/myself/tf_learning/learning_begin/mnist_transfer_model"
    mnist_classifier = tf.estimator.Estimator(model_fn=cnn_model_fn, model_dir=mnist_transfer_model_dir, params={
        "transfer": True,
        "nb_classes": 2,
        "checkpoints": [
            (os.path.join(BASE_DIR, "mnist_model"), True),  # (dir, load_dense_layer)
        ]
    })

    train_input_fn = dataset_input_fn(
        folders=[
            os.path.join("/Users/sunyihuan/Desktop/test/English/Hnd/Img", "Sample041"),  # e
            os.path.join("/Users/sunyihuan/Desktop/test/English/Hnd/Img", "Sample045"),  # i
        ],
        labels=[0, 1],
    ...
    # 第二次迁移学习，训练所有全连接层
    mnist_finetune_model_dir = "/Users/sunyihuan/PycharmProjects/myself/tf_learning/learning_begin/mnist_finetune_model"
    mnist_classifier = tf.estimator.Estimator(model_fn=cnn_model_fn, model_dir=mnist_finetune_model_dir, params={
        "finetune": True,
        "nb_classes": 2,
        "checkpoints": [(os.path.join(BASE_DIR, "mnist_model"), True),  # (dir, load_dense_layer)
                        (os.path.join(BASE_DIR, "mnist_transfer_model"), False)
                        ]
    })
